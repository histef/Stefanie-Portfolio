...
self.addEventListener('fetch', event => {
  ...
  event.respondWith( //tells browser we're handling this request ourselves
    ...
    //gets something out of any cache, starting with the oldest
      caches.match(event.request)
        .then( res => {
        if (res) {
         return res;  //check cache first, if not there, then fetch from network
        } else {
  ...
  //  checks if initial request (from server) fails(no connection)
